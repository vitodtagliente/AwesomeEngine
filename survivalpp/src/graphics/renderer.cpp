...
namespace graphics
{
	...
	void Renderer::submitDrawTexture(Texture& texture, const math::vec2& position, const float angle, const math::vec2& scale, const TextureRect& rect, const TextureFlipMode flip, const Color& color)
	{
		const math::vec2 full_size(pixelsPerUnit * scale.x, pixelsPerUnit * scale.y);
		const math::vec2 size = full_size / 2.f;
		SDL_FRect clip = { rect.x * texture.width(), rect.y * texture.height(), rect.width * texture.width(), rect.height * texture.height() };
		SDL_FRect dest = { m_viewport.x + position.x * pixelsPerUnit - size.x, m_viewport.y - position.y * pixelsPerUnit - size.y, full_size.x, full_size.y };

		SDL_RendererFlip mode;
		switch (flip)
		{
		case TextureFlipMode::Horizontal: mode = SDL_RendererFlip::SDL_FLIP_HORIZONTAL; break;
		case TextureFlipMode::Vertical: mode = SDL_RendererFlip::SDL_FLIP_VERTICAL; break;
		case TextureFlipMode::Both: mode = static_cast<SDL_RendererFlip>(SDL_RendererFlip::SDL_FLIP_HORIZONTAL | SDL_RendererFlip::SDL_FLIP_VERTICAL); break;
		case TextureFlipMode::None:
		default: 
			mode = SDL_RendererFlip::SDL_FLIP_NONE; break;
		}

		SDL_RenderTextureRotated(sdl_renderer, texture.data(), &clip, &dest, static_cast<double>(angle), nullptr, mode);
	}

	void Renderer::submitDrawTexture(Texture& texture, const TextureRect& rect, const Color& color)
	{
		SDL_FRect clip = { rect.x * texture.width(), rect.y * texture.height(), rect.width * texture.width(), rect.height * texture.height()};

		SDL_RenderTextureRotated(sdl_renderer, texture.data(), &clip, nullptr, 0.0, nullptr, SDL_RendererFlip::SDL_FLIP_NONE);
	}
}